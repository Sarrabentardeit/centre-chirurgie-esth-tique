...
export default function NotificationsPage() {
  ...
  const load = useCallback(async (opts?: { silent?: boolean; useCache?: boolean }) => {
    const key = queryKeys.notifications('patient')
    ...
    setError(null)
    try {
      const res = await cachedFetch(key, () => patientApi.getNotifications(), { force })
  ...
  const openNotif = async (notif: UiNotif) => {
    try {
      if (!notif.lu) {
        ...
        setRows((prev) => {
          const next = prev.map((n) => (n.id === notif.id ? { ...n, lu: true } : n))
          setCachedData(queryKeys.notifications('patient'), { ok: true as const, notifications: next })
          return next
        })
      }
