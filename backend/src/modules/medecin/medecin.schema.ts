...
export const rapportSchema = z.object({
  ...
  forfaitPropose:           z.coerce.number({ error: 'Le forfait médical est obligatoire.' }).positive('Le forfait doit être supérieur à 0.'),
  nuitsPreoperatoires:      z.coerce.number({ error: 'Le nombre de nuits préopératoires est obligatoire.' }).int().min(0).max(30),
  nuitsClinique:            z.coerce.number({ error: 'Le nombre de nuits postopératoires est obligatoire.' }).int().min(0).max(60),
  nuitsHotel:               z.coerce.number({ error: 'Le nombre de nuits hôtel est obligatoire.' }).int().min(0).max(60),
  ...
  dureeSejourTunisie:       z.coerce.number().int().min(0).max(180).optional(),
