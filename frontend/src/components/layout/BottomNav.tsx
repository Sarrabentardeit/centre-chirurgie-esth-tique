...
/** 4–5 items les plus importants par rôle (navigation bas mobile). */
const BOTTOM_NAV_ITEMS: Record<UserRole, BottomNavItem[]> = {
  patient: [
    { label: 'Dossier',    href: '/patient/dossier',     icon: FileText },
    { label: 'Formulaire', href: '/patient/formulaire',   icon: ClipboardList },
  ...
  medecin: [
    { label: 'Dashboard',  href: '/medecin/dashboard',    icon: LayoutDashboard },
  ...
  gestionnaire: [
    { label: 'Dashboard',  href: '/gestionnaire/dashboard',       icon: LayoutDashboard },
...
export function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 inset-x-0 z-50 flex lg:hidden border-t border-border bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/90"
        ...
        >
          {({ isActive }) => (
            <>
              ...
              <span className="truncate max-w-[3rem] text-center">{label}</span>
            </>
          )}
        </NavLink>
      ))}
